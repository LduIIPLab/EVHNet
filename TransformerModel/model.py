...
class LFEM(nn.Module):
    ...
    def forward(self, x):
        features_permuted = x.permute(2, 1, 0)
        max_pool = self.max_pool(features_permuted)
        avg_pool = self.avg_pool(features_permuted)

        channel_attention = self.mlp(max_pool) + self.mlp(avg_pool)
        channel_attention = torch.sigmoid(channel_attention)

        spatial_attention = torch.cat([max_pool, avg_pool], dim=0)
        spatial_attention = self.conv(spatial_attention)

        attended_features = features_permuted * channel_attention * spatial_attention
        permuted_tensor = attended_features.permute(2, 1, 0)
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, vis):
        ...
        self.lfem = LFEM(config.hidden_size)

    def cls_taken(self, x):
        y = x[:, 0:1, :]
        return y
    ...
    def forward(self, input_ids):
        ...
        for i in range(4):
            ...
            cls = self.cls_taken(x)
            cls_list.append(cls)
        cls_global = torch.cat(cls_list, dim=1)
        cls_global = cls_global.repeat(1, 12, 1)
        cls_global = torch.cat((cls_global, cls_global[:, :2, :]), dim=1)
        encoded = self.lfem(x)
        x = encoded+cls_global
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, labels=None):
        x, attn_weights_list = self.transformer(x)
        x = x.view(x.size(0), -1)
