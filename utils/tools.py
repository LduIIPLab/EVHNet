...
def config_dataset(config):
    if "Food-101" in config["dataset"]:
        config["topK"] = 1000
        config["n_class"] = 101
    elif config["dataset"] == "Food-172":
        config["topK"] = 1000
        config["n_class"] = 172

    config["data_path"] = "./dataset/" + config["dataset"] + "/"
    if config["dataset"] == "Food-101":
        config["data_path"] = "./dataset/Food-101/"
    config["data"] = {
...
def get_data(config):
    ...
    for data_set in ["train_set", "test", "database"]:
        ...
        dset_loaders[data_set] = util_data.DataLoader(dsets[data_set],batch_size=data_config[data_set]["batch_size"],shuffle=True, num_workers=4)
